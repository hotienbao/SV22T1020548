...
namespace SV22T1020548.Admin.Controllers
{
    ...
    public class EmployeeController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> ToggleWorkingStatus(int id, bool isWorking)
        {
            if (id <= 0)
                return Json(new ApiResult(0, "Mã nhân viên không hợp lệ."));

            bool ok = await HRDataService.UpdateEmployeeWorkingStatusAsync(id, isWorking);
            if (!ok)
                return Json(new ApiResult(0, "Không thể cập nhật trạng thái nhân viên."));

            return Json(new ApiResult(1, "Đã cập nhật trạng thái nhân viên."));
        }
    }
}
