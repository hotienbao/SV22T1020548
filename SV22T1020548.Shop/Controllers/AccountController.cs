...
namespace SV22T1020548.Shop.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(string email, string password, string? returnUrl = null)
        {
            ...
            // Khách hàng bị khóa: không cho đăng nhập vào Shop
            if (customer?.IsLocked == true)
            {
                TempData["ErrorMessage"] = "Tài khoản của bạn đang bị khóa. Vui lòng liên hệ Admin để mở khóa mới có thể mua hàng.";
                TempData["LoginEmail"] = email;
                return RedirectToAction("Login", new { returnUrl });
            }
